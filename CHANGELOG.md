...
- The AI Workspace's tool surface is no longer a fixed list. An ability that declares itself fit for a conversational surface -- and annotates itself `readonly: true`, `destructive: false`, `open_world: false` -- is offered to the assistant alongside the three that ship with it. Admission defaults to refusing: an ability that declares nothing is never offered. The declaration key stays private while [#354](https://github.com/WordPress/ai/issues/354) settles its public shape, so the surface does not change until it lands ([#1003](https://github.com/WordPress/ai/issues/1003)).
