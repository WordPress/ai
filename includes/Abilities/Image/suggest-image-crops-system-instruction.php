...
$aspect_ratios_list = isset( $aspect_ratios ) && is_array( $aspect_ratios ) && ! empty( $aspect_ratios )
...
Coordinate system (matches the WordPress focal point selector):
...
- x and y identify a position within the image. width and height are fractions of the image's full width and height.
...
- If the image has multiple equally important subjects, pick the centroid of the group.
- If the image is a pattern, texture, or has no clear subject, return the visual center (0.5, 0.5).

Crop rules (one entry per requested aspect ratio, in the same order they were requested):
...
- The crop's shape must match the requested ratio. The image's pixel dimensions are unknown to you, so treat the image as square for ratio math: if the requested ratio is W:H, then width / height should equal W / H. The caller will rescale.
- The focal point (x, y) must lie inside the crop window.
- Maximize the crop's area while satisfying the aspect ratio and keeping the primary subject(s) visible. Avoid cutting through faces, heads, or essential parts of the subject. Prefer some breathing room around the subject over tight crops, unless the subject already fills the frame.
INSTRUCTION;
