...
class Provider_Metadata_Registry {
	...
	public static function get_metadata(): array {
		...
		foreach ( $registry->getRegisteredProviderIds() as $provider_id ) {
			...
			$provider_class = is_object( $class_name ) ? get_class( $class_name ) : $class_name;

			if ( ! is_string( $provider_class ) || ! class_exists( $provider_class ) || ! method_exists( $provider_class, 'metadata' ) ) {
				continue;
			}
			...
			$providers[ $metadata->getId() ] = array(
				...
				'models'          => self::get_models_for_provider( $provider_class, $metadata->getId(), $credentials ),
			);
		}
	...
	private static function get_initials( string $name ): string {
		...
		if ( ! is_array( $parts ) || empty( $parts ) ) {
			return strtoupper( (string) substr( $name, 0, 2 ) );
		}
		...
		return (string) substr( $initials, 0, 2 );
	}
	...
	private static function get_models_cache_key( string $provider_id, $credential ): ?string {
		...
		if ( is_array( $credential ) ) {
			...
			return 'wpai_provider_models_' . md5( $provider_id . '|' . (string) $credential );
	}
