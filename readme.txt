...
- The AI Workspace's tool surface is no longer a fixed list. An ability that declares itself fit for a conversational surface - and annotates itself `readonly: true`, `destructive: false`, `open_world: false` - can be offered to the assistant alongside the three that ship with it. Admission defaults to refusing: an ability that declares nothing is never offered. Declaration-based admission is itself switched **off** until [#354](https://github.com/WordPress/ai/issues/354) settles the declaration's public shape, so the shipped surface is still the three curated abilities; `add_filter( 'wpai_workspace_tool_admission_enabled', '__return_true' )`, or the `WPAI_WORKSPACE_TOOL_ADMISSION` constant, turns it on to try it. The declaration and the annotations are both the ability author's self-attestation, and the effect class is a declared intent rather than an enforced property: the site owner's Explorer controls and the execute-time `permission_callback` remain the boundaries ([#1003](https://github.com/WordPress/ai/issues/1003)).
