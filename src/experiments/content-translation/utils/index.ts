 ...
 * @return An object containing the clientId, content, and the attribute the
 *         translated content should be written back to, or null if the
 *         block is not translatable.
 */
export function getTranslatableBlock( block: Block ) {
	const content = getBlockHTML( block );
	const attribute = getEditableTextAttribute( block );
