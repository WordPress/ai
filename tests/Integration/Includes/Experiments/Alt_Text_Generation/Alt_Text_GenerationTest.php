...
class Alt_Text_GenerationTest extends WP_UnitTestCase {
	...
	/**
	 * Test that the bulk script is not enqueued when capability is missing.
	 *
	 * @since x.x.x
	 */
	...
	/**
	 * Test that the bulk script is not enqueued without the query param.
	 *
	 * @since x.x.x
	 */
	public function test_maybe_enqueue_bulk_script_skips_without_query_param(): void {
		$admin_id = self::factory()->user->create( array( 'role' => 'administrator' ) );
		wp_set_current_user( $admin_id );
	...
	/**
	 * Test that the bulk script is not enqueued when attachment IDs are empty.
	 *
	 * @since x.x.x
	 */
	public function test_maybe_enqueue_bulk_script_skips_with_empty_ids(): void {
		$admin_id = self::factory()->user->create( array( 'role' => 'administrator' ) );
		wp_set_current_user( $admin_id );

		$_GET['ai_bulk_alt_text']  = '1';
		$_GET['ai_attachment_ids'] = '';
